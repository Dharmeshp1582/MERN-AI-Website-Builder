...
import { BotIcon, EyeIcon, Loader2, SendIcon, UserIcon } from 'lucide-react';
...
const Sidebar = ({isMenuOpen, project,setProject, isGenerating, setIsGenerating} : SidebarProps) => {
   ...
   {console.log("conversation is ",project.conversation)}
          ...
          if(isMessage){
            ...
            return (
              <div key={msg.id} className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
               ...
               <div className={` max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ? 'bg-linear-to-r from-indigo-500 to-indigo-600 rounded-tr-none text-white' : 'text-gray-100 rounded-tl-none bg-gray-800'}`}>
                {msg.content} 
               </div>
...

        {
          isGenerating && (
            <div className='flex items-start gap-3 justify-start'>
             <div className='w-8 h-8 rounded-full bg-linear-to-br from-indigo-700 flex items-center justify-center'>
              <BotIcon className='size-5 text-white ' />
             </div>
     ...
     <div ref={messageRef}  />
      ...
      <form onSubmit={handleRevisions} className='m-3 relative'>
       ...
       <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-60'>
        {isGenerating ? ( <Loader2 className='size-7 p-1.5 text-white animate-spin' /> ) : ( <SendIcon className='size-7 p-1.5 text-white' />)}
       </button>
