          ...
          if(isMessage){
            ...
            return (
              <div key={msg.id} className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
               ...
               <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ? 'bg-linear-to-r from-indigo-500 to-indigo-600 rounded-tr-none text-white' : 'text-gray-800 rounded-tl-none bg-gray-800'}`}>
                {msg.content}
               </div>
              </div>
            )
