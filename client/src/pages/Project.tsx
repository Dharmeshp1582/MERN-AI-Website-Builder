...
const Project = () => {
  ...
  return project ? (
   ...
   <div className='flex items-center gap-2 sm:min-w-90 text-nowrap'>
     ...
     {isMenuOpen ? (
       <MessagesSquareIcon onClick={()=> setIsMenuOpen(false)} className='size-6 cursor-pointer ' />
  ...
  <button onClick={togglePublish} className='bg-linear-to-br from-indigo-700 to-indigo-600 hover:from-indigo-600 hover:to-indigo-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors'>
    {project.isPublished ? <EyeOffIcon size={16}/> : <EyeIcon size={16}/>} {project.isPublished ? "Unpublish" : "Publish"}
  </button>
   </div>
      </div>
